...
/*# define gf256mul(x,y)      					\
        ...
        ((y>>4 & 1) * xtime(xtime(xtime(xtime(x)))))) //This part is understood and taken from git for gf multiplication optimization
...
const uint8_t gf_antilog_table[256] ={
...
{
  return ((x<<1) ^ (((x>>7) & 1) * 0x1b)); ///This part is understood and taken from git for gf multiplication optimization
}
*/
...
uint8_t gf256mul(uint8_t a,uint8_t b) {
 ...
 void inverse_mix_coloumns(uint8_t* s){
	for (uint8_t i=0 ; i<4 ; i++){
		...
		s[index] =  gf256mul(s[index],0x0e) ^ gf256mul(temp1,0x0b) ^ gf256mul(temp2,0x0d) ^ gf256mul(s[index+3],0x09);
	 s[index+1] =  gf256mul(temp0,0x09) ^ gf256mul(temp1,0x0e) ^ gf256mul(temp2,0x0b) ^ gf256mul(s[index+3],0x0d);
	 s[index+2] =  gf256mul(temp0,0x0d) ^ gf256mul(temp1,0x09) ^ gf256mul(temp2,0x0e) ^ gf256mul(s[index+3],0x0b);
